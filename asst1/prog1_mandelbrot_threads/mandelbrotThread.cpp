...
void workerThreadStart(WorkerArgs* const args) {
    unsigned int heightPerThread =
        (args->height + args->numThreads - 1) / args->numThreads;

    unsigned int startRow = args->threadId * heightPerThread;
    unsigned int numRows = (startRow + heightPerThread > args->height) ? (args->height - startRow) : heightPerThread;
    double startTime = CycleTimer::currentSeconds();
    mandelbrotSerial(args->x0, args->y0, args->x1, args->y1, args->width,
                     args->height, startRow, numRows, args->maxIterations,
                     args->output);
    double endTime = CycleTimer::currentSeconds();
    printf("thread %d rows [%u, %u) took %.3f ms\n", args->threadId,
